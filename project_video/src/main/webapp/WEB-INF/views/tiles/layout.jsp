...
<script type="text/javascript">
				...
				$
						.ajax({
							...
							success : function(data) {
									...
									for (j; j < length; j++) {
										...
										if (data[j] == null) {
											...
											if (k < l + 3) {
												...
												output += "<span class='favList-name'>"
														+ vtitle + "</span>";
												output += "</div>"
												k++;
											} else {
								...
								$(".favorites-inner").children().first()
										.addClass("active");
									...
									for (j; j < length; j++) {
										...
										if (data.mlist[j] == null) {
											...
											if (k < l + 3) {
												...
												output += "<img  src='"+thumImg+"'>"
												output += "<span class='recentlyList-name'>"
														+ vtitle + "</span>";
												output += "</div>"
												k++;
											} else {
				...
				$
						.ajax({
							...
							success : function(data) {
								...
								$(".recently-left-inner").children().first()
										.addClass("active");
	...
	function myRecentlyList(reNum) {
		...
		window.location.href = "myRecentlyVideo.do?reNum=" + reNum + "&cookie="
				+ cookie;
	...
	/* 게시물 클릭  */
	function videoClick(videoidData, vnumData, vurlData, bnumData, clickObjeck) {

		var scrollH = $(window).height();
		//alert(scrollH);
		var videoH = scrollH / 2;
	...
	function fnMove(vnum) {
		...
		var offset = $(".row-sm-" + vnum).offset();
		var moveTopH = $(window).height() / 4;
		//alert(offset.top);
		//alert(moveTopH);
		$('html, body').animate({
			scrollTop : offset.top - moveTopH
		}, 300);
	...
	function condition() {
		...
		if (width <= 991) {
			$(".paging-btn").load(
					"mainView.do?search=" + searchTxt + "&sort=" + sortCheck
							+ "&gnum=" + gnumCheck + " #paging-btn-group");
			$("#main-content").load(
					"mainView.do?search=" + searchTxt + "&sort=" + sortCheck
							+ "&gnum=" + gnumCheck + " #accordion");
		} else {
			$(".paging-btn").load(
					"mainView.do?search=" + searchTxt + "&sort=" + sortCheckLg
							+ "&gnum=" + gnumCheck + " #paging-btn-group");
			$("#main-content").load(
					"mainView.do?search=" + searchTxt + "&sort=" + sortCheckLg
							+ "&gnum=" + gnumCheck + " #accordion");
		...
		if (view == "mainView.do") {
			...
			case "prev":
				...
				if (nowPage != 1) {
					...
					$(".paging-btn").load(
							view + "?nowPage=" + startPrev + "&cntPerPage="
									+ cntPerPage + "&sort=" + sort + "&gnum="
									+ gnum + "  #paging-btn-group");
					$("#main-content").load(
							view + "?nowPage=" + startPrev + "&cntPerPage="
									+ cntPerPage + "&sort=" + sort + "&gnum="
									+ gnum + " #accordion");
			...
			case "next":
				if (nowPage != lastPage) {
					//alert("다름");
					$(".paging-btn").load(
							view + "?nowPage=" + startNext + "&cntPerPage="
									+ cntPerPage + "&sort=" + sort + "&gnum="
									+ gnum + "  #paging-btn-group");
					$("#main-content").load(
							view + "?nowPage=" + startNext + "&cntPerPage="
					...
					success : function(result) {
						...
						for ( var i in result) {
							...
							if (result[i].rnum == 0) {
								...
								output += "<div class='reply-replyer-box'>"
										+ result[i].replyer;
								...
								output += "</div>";
								output += "<div class='reply-text-box'>"
										+ result[i].replyText
										+ "<button class='btn badge reReply-badge-btn' onclick='reCommentBox("
										+ result[i].rpnum + ","
										+ result[i].rnum + "," + result[i].bnum
										+ ",0)'>답글</button></div>";
								output += "</div>";
								output += "</div>";
	...
	function reCommentBox(rpnum, rnum, bnum, rnum) {
		...
		output += "<button type='button' name='boardNum-"
				+ bnum
				+ "' class='btn reReply-btn col-lg-7 col-7' onclick='insertReReply("
				+ bnum + "," + rpnum + ")'>답글 입력</button>"
		output += "<button type='button' class='btn reReply-cancle-btn col-lg-5 col-5' onclick ='cancel("
				+ rpnum + "," + bnum + ")'>취소</button>"
		output += "</div></div>"
