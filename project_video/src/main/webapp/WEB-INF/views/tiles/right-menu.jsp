<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
	

<button class="btn btn-secondary dropdown-toggle right-menuBtn" data-display="static" aria-haspopup="true" aria-expanded="false">
	<span class="right-menu-title"> 최근 본 게시물 </span>
</button>

<div id="recentlyNone"><h1>최근 본 게시물이 없습니다.</h1></div>

<div id="nav-slide" class="carousel vert slide" data-ride="carousel" data-interval="false">
	
	<a class="carousel-control-up" href="#nav-slide" role ="button" data-slide="prev">
		<span class="fas fa-angle-up  fa-2x" aria-hidden="true"></span>
		<span class="sr-only">Previous</span>
	</a>

	<!-- Wrapper for slides -->
	<div class="carousel-inner slide-body" >
		
		
	</div>
	
	<a class="carousel-control-down" href="#nav-slide" role ="button" data-slide="next">
		<span class="fas fa-angle-down fa-2x" aria-hidden="true"></span>
		<span class="sr-only">Next</span>
	</a>

</div>

<script type="text/javascript">
	
	$(".carousel-control-up").on("click",function(){
		
		if($(".slide-body").children().first().hasClass("active")){
			
			$(".carousel-control-up").removeAttr("href");
		}else{
			
			$(".carousel-control-up").attr("href","#nav-slide");
		}
	})
	$(".carousel-control-down").on("click",function(){
		
		if($(".slide-body").children().last().hasClass("active")){
			
			$(".carousel-control-down").removeAttr("href");
		}else{
			
			$(".carousel-control-down").attr("href","#nav-slide");
		}
	})
	
</script>